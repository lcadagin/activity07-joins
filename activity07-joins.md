...
Yes, there are about 14 La Quinta locations outside of the USA (looks
like they are in Mexico and Central America). No, there are no Denny’s
...
**Response**: In my code above, I filtered out all US states and DC to
see if there were any foreign locations for Dennys and Laquinta.
...
**Response**: There are no foreign Denny’s locations in the dataset.
...
**Response**: Delaware has the fewest Denny’s locations (only 1)
...
**Response**: Maine has the fewest La Quinta locations (only 1).

Is this surprising to you? Why or why not?

**Response**: I was surprised that Delaware has only one Dennys. I would
have expected there to be more restaurants in a such a populated state.
I am not very surprised there is only one La Quinta in Maine. It is a
relatively small and sparsely populated state.
...
**Response**: Washington DC, Rhode Island, California, Connecticut,
Florida, and Maryland have the most Denny’s locations per thousand
square miles.
...
**Response**: Rhode Island, Florida, Connecticut, and Marylnad have the
most La Quintas locations per thousand square miles.
...
**Response**: If we used any sort of join, some of the data could be
lost. A bind\_rows command will ensure that all data is combined.

If we were able to use a mutating join, what would this output would
look like? Why would this not be helpful?

**Response**: The output of a mutating join would show all the columns
from the left table, a single column for the column we join on, followed
by all the columns from the second table. This is not helpful because we
want addresses, long, lat, etc. from both tables to be in the same
columns.
...
![](activity07-joins_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->

**Response**: No, it does not appear that Mitch Hedberg’s joke holds up
very well in MI. There are only two cities that a LaQuinta and Denny’s
...
![](activity07-joins_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->

**Response**: Mitch Hedberg’s joke holds up a little better in Texas as
there are many more Denny’s near La Quintas.
